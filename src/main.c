...
static void print_help(const char *argv0) {
    fprintf(stderr,
        ...
        "  --print-state          full ephemeris + comets + asteroids +\n"
        "                         bundled satellites (text columns)\n"
        "  --print-state --json   same as JSON (pipeable into jq)\n"
        "  --next <body>          next rise time for a planet/comet/asteroid/\n"
        "                         DSO/bundled satellite (incl. catalog number)\n"
        ...
        "  --validate             run internal sanity tests (9 cases against\n"
        "                         Meeus + JPL Horizons; PASS/FAIL with deltas)\n"
        "  --snapshot [cols rows] render one astro frame to stdout and exit\n"
        "  --update-tle           refresh bundled satellite TLEs from CelesTrak\n"
        "                         (opt-in; rate-limited 2h; writes user cache)\n"
        "\n"
        ...
        "              /  search by name/catalog; jumps + auto-tracks\n"
        "              T  track mode (cursor follows nearest body)\n"
        "              g grid, l lines, d deep-sky, a aurora, i satellites,\n"
        "              t trails, m geo/helio, s star backdrop,\n"
        "              c cursor (hjkl, Esc), or click to pick + track.\n",
        argv0);
}
